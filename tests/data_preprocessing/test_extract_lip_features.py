...
class TestExtractLipFeatures:
    @pytest.fixture
    def setup_landmarks_and_smile(self, lip_corner_indices):
        li, ri = lip_corner_indices

        landmarks_df = _df_with_lip_corners(
            frames=[0, 1, 2, 3],
    ...
    def test_happy_path_calls_extract_features_with_expected_df(self, monkeypatch, setup_landmarks_and_smile):
        ...
        fps = 25.0
        result = extract_lip_features(PathLikeDummy("landmarks.csv"), PathLikeDummy("smile.csv"), fps)
        ...
        D = (
            normalized_amplitude_signal_of_lip_corners(landmarks_df)
            .set_index("frame_number")
            .loc[merged_frames, "normalized_amplitude_signal_of_lip_corners"]
    ...
    def test_raises_when_reading_files_fails(self, monkeypatch):
        ...
        with pytest.raises(Exception):
            extract_lip_features(PathLikeDummy("landmarks.csv"), PathLikeDummy("smile.csv"), 30.0)
